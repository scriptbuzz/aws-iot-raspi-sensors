...
```mermaid
flowchart LR
    %% Define Edge Device Services
    subgraph Edge [Raspberry Pi Device]
        Sensors([Sense HAT Sensors])
        App[[basicPubSub.py]]
        Display[[LED Matrix UI]]
    end

    %% Define AWS Cloud Services
    subgraph Cloud [AWS Cloud Architecture]
        MQTT((AWS IoT MQTT Topic))
        Rule{AWS IoT Rule}
        Lambda[[AWS Lambda]]
        SNS[[Amazon SNS]]
    ...
    %% Edge internals
    Sensors -->|Reads Data| App
    App -.->|Updates| Display

    %% Edge to Cloud Communication
    App -->|Publishes Data Payload| MQTT

    %% Cloud Internals
    MQTT -->|Triggers| Rule
    
    Rule -->|Stores Metrics| DynamoDB
    Rule -->|Invokes| Lambda

    %% Lambda Operations
    Lambda -->|Evaluates Threat / Breached?| SNS
    Lambda -->|Publishes Command (WARN/ALARM/GO)| MQTT

    %% Cloud back to Edge Communication
    MQTT -->|Subscribes to Commands| App

    %% Styling
    classDef aws fill:#FF9900,color:#fff,stroke:#fff,stroke-width:2px;
